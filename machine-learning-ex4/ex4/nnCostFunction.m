...
%得到代价值
%因为计算形式问题，先算不带正则化的
for    i=1:m%累计所有样本的代价值
    J=J+yNum(i,:)*log(a3(i,:))'+(1.-yNum(i,:))*log(1.-a3(i,:))';%单个样本的代价值
end
J=J/(-m);
%计算正则化项带来的代价值-------计算各层各单元各参数的值--即所有参数的平方和
regularization=lambda/(2*m)*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
J=J+regularization;


%使用反向传播计算导数
%--------------以向量化形式实现步骤（1）至（4）的循环（累积求和）--------------%
d3=a3-yNum;
